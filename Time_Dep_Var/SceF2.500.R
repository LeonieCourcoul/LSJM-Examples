### Script simulations - Scenario F2 - 500
...
for(rep in 1:nb.simu){
  ...
  lsmm.simu <- lsmm(formFixed = y ~ visit , formRandom = ~ visit ,
                    ...
                    formFixedVar = ~ 1  , formRandomVar = ~ 1, correlated_re = F,
...
colnames(CR1) <- c("shape","alpha0","alphaCV","alphaSlope","alphaSigma",
                   "beta0","beta1","M0",
                   "chol1","chol2","chol3","chol4",
                   "var_b0","cov_b0b1","var_b1","var_tau0")
...
print(Simu_analysis[c(1:8,13:16),])
View(Simu_analysis[c(1:8,13:16),])
