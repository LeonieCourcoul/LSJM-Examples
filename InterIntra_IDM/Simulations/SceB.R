...
Gen_data <- function(n, B, mu_sigma, mu_kappa, nb.mesures, beta0, beta1,
  ...
  while(i<=n){
    ...
    T1i <- A_0i + runif(1, 3.8, 4.2)
    T2i <- T1i + runif(1, 3.8, 4.2)
    T4i <- T2i + runif(1, 3.8, 4.2)
    T7i <- T4i + runif(1, 3.8, 4.2)
    visit <- c(A_0i, T1i,T2i, T4i, T7i)
    visit <- (visit-65)/10
    visit <- rep(visit, each = nb.mesures)
    #browser()
    data_long_i <- c()
    data_long_i <- as.data.frame(cbind(rep(i, length(visit)),
                                       rep(c("S0","S1","S2","S4","S7"), each = nb.mesures),
                                       sort(visit)))
